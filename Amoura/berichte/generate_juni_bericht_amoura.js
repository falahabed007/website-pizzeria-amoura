/**
 * Lokaler Generator: Amoura Monatsbericht Juni 2026 (Teilmonat 01.–12.06.)
 * Datenquelle: Live-Admin-API (kein lokaler DB-Zugang nötig)
 * Ausführen: NODE_PATH=<pfad-zu-node_modules> node generate_juni_bericht_amoura.js
...
// ── Zeitraum: 01.–12. Juni 2026 ──────────────────────────────────
const JAHR  = 2026;
const MONAT = 5;          // 0-basiert: 5 = Juni
const TAG_VON = 1;
const TAG_BIS = 12;
...
async function main() {
  ...
  const onlineM    = orders.filter(o => o.payment !== 'bar');
  ...
  const titelZeitraum = `Pizzeria Amoura  ·  ${vonBis}  (Teilmonat)`;
  const monatsPdf = await generatePdf(doc => {
    ...
    doc.font('Helvetica').fontSize(7).fillColor('#bbb')
      .text(`FlueVate · Abed Rachman Falah · Zur Goldbrede 30 · 59269 Beckum  ·  Monatsbericht ${monat} (01.–12.)`, PDF_M, 820, { width: PDF_W, align: 'center' });
  });

  const outDir = path.join(__dirname, '..', 'PDFs'); // Berichte weiterhin ins Repo-Root/PDFs
  if (!fs.existsSync(outDir)) fs.mkdirSync(outDir, { recursive: true });
  const file1 = path.join(outDir, `Juni_2026_Amoura_Monatsbericht_bis-12.pdf`);
