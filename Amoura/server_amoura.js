...
// ─── PDF Layout-Helpers ───────────────────────────────────────────────────
const PDF_M  = 50;          // margin
const PDF_W  = 495;         // content width
const PDF_PW = 595;         // page width
const PDF_FT = 810;         // footer y
const PDF_SV = 0.99;        // service fee
const PDF_PR = 0.05;        // provision rate
...
function pdfBarRechnung(doc, barOrders, barStats, zeitraum, rgnr) {
  ...
  doc.font('Helvetica').fontSize(9).fillColor('#333').text('Servicegebühren', PDF_M+8, s1y+5).text(`${barOrders.length} × 0,99 €`, PDF_M+200, s1y+5, { width:140, align:'right' });
...
cron.schedule('59 23 * * 0', async () => {
  try {
    ...
    const barSvc      = barOrders.reduce((s,o) => s+(o.serviceFee||0.99), 0);
    const barNetto    = barOrders.reduce((s,o) => s+(o.total||0), 0) - barSvc;
    const barProv     = barNetto * 0.05;
    ...
    const berichtPdf = await generatePdf(doc => {
      ...
      doc.y += 14;
      pdfTableRow(doc, [[`Servicegebühren  (0,99 € × ${orders.length})`, PDF_M+8, PDF_W-80, 'left'], [pdfFmt(svcFees), PDF_M+2, PDF_W-4, 'right']], false);
  ...
  <div style="padding:24px 28px">
    ...
    <p style="font-size:11px;color:#aaa;margin-top:8px">Anbei der Wochenbericht mit Kundenliste${barOrders.length > 0 ? ' und Bar-Rechnung' : ''}.</p>
        ...
        attachments: [{ filename: `KW${kw}_${now.getFullYear()}_Wochenbericht.pdf`, content: berichtPdf.toString('base64') }],
      });
    }
    ...
    if (process.env.OWNER_EMAIL) {
      await getResend()?.emails.send({
        ...
        html: `<p style="font-family:Arial,sans-serif;color:#555">Anbei Rechnung <b>${rechnungNr}</b> und Wochenbericht KW ${kw} / ${now.getFullYear()}.</p>
        ...
        attachments: [
          ...
          { filename: `KW${kw}_${now.getFullYear()}_Wochenbericht.pdf`, content: berichtPdf.toString('base64') },
        ],
      });
    }
...
// ═══════════════════════════════════════════════════════════════════
// MONATSBERICHT (täglich 23:58, nur am letzten Tag des Monats)
// ══════════════════════════════════════════════════════════════
cron.schedule('58 23 * * *', async () => {
  ...
  try {
    ...
    const barSvcM     = barOrdersM.reduce((s,o) => s+(o.serviceFee||0.99), 0);
    const barNettoM   = barOrdersM.reduce((s,o) => s+(o.total||0), 0) - barSvcM;
    const barProvM    = barNettoM * 0.05;
    ...
    const monatsPdf = await generatePdf(doc => {
      pdfColorBox(doc, `Monatsbericht ${monat}`, `Pizzeria Amoura  ·  ${vonBis}`, '#1a1a2e');
      pdfKacheln(doc, [
        ['Bestellungen gesamt', `${orders.length}`,                              '#1a1a2e'],
  ...
  <div style="padding:24px 28px">
    ...
    <p style="font-size:11px;color:#aaa;margin-top:8px">Anbei der Monatsbericht mit Kundenliste${barOrdersM.length > 0 ? ' und Bar-Rechnung' : ''}.</p>
        ...
        attachments: [{ filename: `${monat.replace(' ','_')}_Monatsbericht.pdf`, content: monatsPdf.toString('base64') }],
      });
    }
    ...
    if (process.env.OWNER_EMAIL) {
      await getResend()?.emails.send({
        ...
        html: `<p style="font-family:Arial,sans-serif;color:#555">Anbei Rechnung <b>${rechnungNr}</b> und Monatsbericht <b>${monat}</b>.</p>
        ...
        attachments: [
          ...
          { filename: `${monat.replace(' ','_')}_Monatsbericht.pdf`,                    content: monatsPdf.toString('base64') },
        ],
      });
    }
