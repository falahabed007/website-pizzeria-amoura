...
async function printOrder(order) {
  ...
  const response = await fetch('https://api.printnode.com/printjobs', {
    ...
    body: JSON.stringify({
      ...
      title: `ATAS Bestellung #${order.orderNum}`,
...
function buildReceipt(order) {
  ...
  receipt += 'Doener & Pizza Beckum' + LF;
  ...
  if (order.deliveryFee && order.deliveryFee > 0) {
    receipt += padLine('Fahrtkosten:', order.deliveryFee.toFixed(2).replace('.', ',') + ' EUR') + LF;
  }
  ...
  receipt += SEPARATOR;
  receipt += CENTER + 'Vielen Dank fuer Ihre Bestellung!' + LF;
  receipt += 'Ihr ATAS Team' + LF;
  receipt += LF + LF + LF;
  receipt += CUT;
